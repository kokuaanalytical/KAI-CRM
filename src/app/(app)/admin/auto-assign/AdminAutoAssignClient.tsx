...
export default function AdminAutoAssignClient() {
  ...
  const [rules, setRules] = useState<
    Array<{ state_code: string; owner_user_id: string; owner_email: string }>
  >([]);
  ...
  async function loadReps() {
    ...
    toast({
      title: "Failed to load reps",
      description: r1.error?.message ?? r2.error?.message ?? "Unknown",
    });
  }
  ...
  async function loadRules() {
    const rs = await supabase
      ...
      .select("state_code,owner_user_id")
      .order("state_code");
  ...
  async function upsertRule() {
    ...
    if (role !== "admin") {
      toast({
        title: "Admins only",
        description: "You can view rules but only admins can edit.",
      });
      return;
    }
    ...
    toast({
      title: "Rule saved",
      description: `${stateCode} → ${reps.find((r) => r.id === ownerId)?.email ?? ownerId}`,
    });
